...
{
    // Core/Controllers/BattleCalculator.cs
    // Dependencies:
    // - Models/Army.cs
    // - Models/Region.cs
    ...
    namespace WarRegions.Controllers
    {
        public class BattleCalculator
        {
            ...
            private int DistributeDamageToUnits(Army army, int totalDamage)
                ...
                // Simple damage distribution - in real implementation, use more sophisticated logic
            ...
            private void LogBattleResult(BattleResult result, int attackerPower, int defenderPower)
                ...
                Location: {result.BattleRegion.RegionName}
                Combatants: {result.Attacker.ArmyName} ({attackerPower}) vs {result.Defender.ArmyName} ({defenderPower})
                Winner: {(result.AttackerWon ? result.Attacker.ArmyName : result.Defender.ArmyName)}
                Casualties: {result.AttackerCasualties} (Attacker) / {result.DefenderCasualties} (Defender)
                Rewards: {result.SilverReward} silver, {result.GoldReward} gold
                """);
            public BattleResult SimulateBattle(Army attacker, Army defender, Region battleRegion, int simulations = 1000)
                ...
                [BATTLE] Simulation Results ({simulations} runs):
                Attacker Win Rate: {winRate:P1}
                Average Attacker Casualties: {totalAttackerCasualties / simulations}
                Average Defender Casualties: {totalDefenderCasualties / simulations}
                Recommended: {(winRate > 0.7 ? "ATTACK" : winRate < 0.3 ? "RETREAT" : "RISKY")}
