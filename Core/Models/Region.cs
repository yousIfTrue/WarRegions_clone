...
namespace WarRegions.Core.Models
{
    ...
    public class Region
    {
        ...
        // ✅ الطرق المطلوبة التي أضفتها فقط
        public int GetCombatBonus()
        {
            return DefenseBonus; // نستخدم الـ DefenseBonus الموجود
        }
        
        public int GetDefenseBonus()
        {
            return DefenseBonus; // نفس القيمة
        }
        
        public bool CanTraverse(MovementType movementType)
        {
            if (!IsPassable) return false;
            
            // منطق إضافي بناءً على نوع الحركة
            switch (Terrain)
            {
                case TerrainType.Mountains:
        ...
        {
            return CalculateMovementCost(movementType); // نستخدم الدالة الموجودة
        }
